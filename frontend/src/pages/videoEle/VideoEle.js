...
function VideoEle({ addToAttendance, attendance, people, setPeopleCount }) {
  ...
  function markPresent(id) {
    ...
    if (student) {
      addToAttendance((prev) => {
        ...
        return [
          ...prev,
          { ...student, time: new Date().toLocaleString(), isPresent: true },
        ];
      });
    }
  }
  ...
  async function startFaceRecognition() {
    ...
    try {
      ...
      setInterval(async () => {
        ...
        captureCount+=1;
        countOfPerson += Math.ceil(detections.length/captureCount);
       
        setPeopleCount(countOfPerson);
  ...
  function getSlots() {
    ...
    return JSON.stringify(hour) + "-" + JSON.stringify(hour + 1);
  }
  ...
  function setTime(time) {
    setTimeout(() => {
      ...
      const data = {
        slot,
        // time: new Date(),
        hospitalId: "SMS Hospital",
        count: countOfPerson,
      };
      console.log(data);
      axios
        .post(`http://localhost:5000/api/update/${slot}`, data)
        .then((res) => {
        ...
        .catch((err) => {
          console.log("error in seding data", err);
          window.location.href='/'
          // window.location.reload();
        });
    }, time);
      ...
      <div>
        <button
          onClick={() => {
            ...
            startFaceRecognition();
            setTime(
              Number(document.getElementById("TimerValue").value) * 60000
            );
          }}
        >
          <p id="start">Start</p>
